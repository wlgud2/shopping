...
<head>
  ...
  function idCheck(id){//id 중복확인
	  if(id==''){
		  alert("아이디를 입력하세요");
		  document.frm.id.focus();
	  }else{
		  var url = "idcheck";//이 url로 이동//membercontroller에 설정
		  var param = "id="+id;
		  
		  $.get(url, param, function(data, textStatus) {//.get: ajax처리하는 명령어 중 하나. get방식으로 비동기 통신요청

			  $("#idcheck").text(data.str);//id가 idcheck인 것에 보내기
		  })
	  }
  } 
...
function inCheck(f){//빈값있는지 확인
	...
	if(f.passwd.value != f.repasswd.value){
		alert("비번과 비번확인이 일치하지 않습니다.");//비밀번호 길이 설정도 할 수 있으면 해
		f.passwd.value="";
		f.repasswd.value="";
		f.passwd.focus();
		return false;
